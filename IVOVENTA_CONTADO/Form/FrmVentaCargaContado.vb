'Imports PrinterTxt
Public Class FrmVentaCargaContado


    '' Es una copia de la venta carga asegura, tiene que dejar libre las modificaciones establecidas.
    '' No debe incluir lo siguiente 
    '' Mensajeria 
    '' Permitir ingresar Fecha, Monto 
...
#End Region
    ...
    Private Sub FrmVentaCargaContado_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Activated
    End Sub
    ...
    Private Sub txtiWinDestino_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txtiWinDestino.Validating
        Try
            ...
            'hlamas
            If txtiWinDestino.ReadOnly Then Exit Try
            ...
            If idUnidadAgencias >= 0 Then
                ...
                If idUnidadAgencias > 0 Then
                    ...
                    If idUnidadAgencias <> iOficinaDestino Then
                        ...
                        'For i As Integer = 0 To dtGridViewArticulo.Rows.Count - 1
                        ' dtGridViewArticulo(5, i).Value() = "0.00"
                        ' dtGridViewArticulo(4, i).Value() = ""
                        ' dtGridViewArticulo(3, i).Value() = ""
                        ' dtGridViewArticulo(2, i).Value() = ""
                        ' Next

                        'hlamas
                        'dtGridViewArticulo.Rows.Clear()
                        txtSub_Total.Text = ""
                        txtMonto_IGV.Text = ""
                        txtCosto_Total.Text = ""
                    ...
                    'If dtoUSUARIOS.iIDAGENCIAS <> Int(ObjVentaCargaContado.rstVarAgencias.Fields(0).Value) Then
                    '    lidagencias = dtoUSUARIOS.iIDAGENCIAS
                    'else 
                    '    lidagencias = Int(ObjVentaCargaContado.rstVarAgencias.Fields(0).Value)
                    'End If
                    'ModuUtil.LlenarComboIDs(ObjVentaCargaContado.rstVarAgencias, cmbAgenciaVenta, ObjVentaCargaContado.coll_AgenciasVenta, lidagencias)
                    '
...
7:                  txtDocCliente_Remitente.Focus()
                    TipoComprobante = 2
                    If ObjVentaCargaContado.fnNroDocuemnto(TipoComprobante) = True Then
                        txtSERIE.Text = RellenoRight(Mro_Digitos_SerieVentas, ObjVentaCargaContado.Serie)
                        txtNroDocFACBOL.Text = RellenoRight(Mro_Digitos_Ventas, ObjVentaCargaContado.NroDoc)
    ...
    Public Function fnTarifario() As Boolean
        ...
        Try
            ...
            ElseIf objGuiaEnvio.fnTARIFA_PUBLICA_CARGA1() = True Then
                ...
                End If
                'Else    'cliente tiene descuento
                'If objGuiaEnvio.fnTARIFA_PUBLICA_CARGA1() = True Then
                '    bTarifarioGeneral = True
                '    bContado = objGuiaEnvio.bContado
                '    tarifa_Peso = objGuiaEnvio.iTarifa_Publica_Monto_Peso
                '    tarifa_Volumen = objGuiaEnvio.iTarifa_Publica_Monto_Volumen
                ...
                '    flag = True
                '    bControl_Tarifas = True
                'ElseIf objGuiaEnvio.fnLISTA_TARIFA_CLIENTE() = True Then
                '    bTarifarioGeneral = False
                '    bContado = False
                '    tarifa_Peso = objGuiaEnvio.iTarifa_Publica_Monto_Peso
                '    tarifa_Volumen = objGuiaEnvio.iTarifa_Publica_Monto_Volumen
    ...
    Private Sub txtCosto_Total_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtCosto_Total.KeyPress
        Try
            Dim tb As TextBox = CType(sender, TextBox)
            Dim chr As Char = e.KeyChar
            If IsNumeric(e.KeyChar) And Not e.KeyChar = "-" Then
    ...
    Private Sub txtCosto_Total_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtCosto_Total.TextChanged
        Try
            If bControlFiscalizacion = True Then
                ...
                Dim dTotal As Double = IIf(Me.txtCosto_Total.Text.Trim.Length = 0, 0, Me.txtCosto_Total.Text)
                Dim dSubtotal As Double = dTotal / (1 + dtoUSUARIOS.iIGV / 100)
                Dim dImpuesto As Double = dSubtotal * (dtoUSUARIOS.iIGV / 100)

                Me.txtSub_Total.Text = Format(dSubtotal, "#,###,###0.00")
                Me.txtMonto_IGV.Text = Format(dImpuesto, "#,###,###0.00")
                '           
            End If
        Catch ex As Exception
    ...
    Private Sub txtDNIContacto_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txtDNIContacto.Validating
        Try
            If flagPCEVALIDADOC = False Then
                If ObjVentaCargaContado.fnSP_BUSCAR_PERSONACONTACTOS(txtDNIContacto.Text) = True Then
...
FINAL:
    ...
    Public Function fnGrabar() As Boolean
        ...
        Try
            ...
            ObjVentaCargaContado.v_SERIE_FACTURA = Int(txtSERIE.Text)
            ...
            Dim dTotal As Double = txtCosto_Total.Text
            Dim dSubtotal As Double = dTotal / (1 + dtoUSUARIOS.iIGV / 100)
            Dim dImpuesto As Double = dSubtotal * (dtoUSUARIOS.iIGV / 100)
            ObjVentaCargaContado.v_MONTO_SUB_TOTAL = Format(dSubtotal, "#,###,###0.00")
            ObjVentaCargaContado.v_MONTO_IMPUESTO = Format(dImpuesto, "#,###,###0.00")
            ...
            Dim dTotal1 As Double = ObjVentaCargaContado.v_TOTAL_COSTO
            Dim dSubtotal1 As Double = dTotal1 / (1 + dtoUSUARIOS.iIGV / 100)
            Dim dImpuesto1 As Double = dSubtotal1 * (dtoUSUARIOS.iIGV / 100)
            ObjVentaCargaContado.v_MONTO_SUB_TOTAL = Format(dSubtotal1, "#,###,###0.00")
            ObjVentaCargaContado.v_MONTO_IMPUESTO = Format(dImpuesto1, "#,###,###0.00")
    ...
    Private Function fnVERDATOSVENTACONTADO(ByVal idControl As Integer) As Boolean
        ...
        Try
            ...
            If flagPCEDatos = True Then
                txtDocCliente_Remitente.Text = ObjVentaCargaContado.v_NRO_DOC_REMITENTE
            ...
            Else
                txtDocCliente_Remitente.Text = ObjVentaCargaContado.v_NRO_DNI_RUC
            ...
            If ObjVentaCargaContado.dt_cur_Documentos.Rows.Count > 0 Then
                ...
                Try
                    ...
                    For Each fila As DataRow In ObjVentaCargaContado.dt_cur_Documentos.Rows 'ds.Tables(3).Rows
                        If IsDBNull(fila.Item("PORCEN")) Then
                            iContador += 1
            ...
            'If ObjVentaCargaContado.v_IDTIPO_COMPROBANTE = 3 Or ObjVentaCargaContado.v_IDTIPO_COMPROBANTE = 85 Then
            If ObjVentaCargaContado.v_MONTO_MINIMOs > 0 Then
    ...
    Public Function fnTotalPago() As Boolean
        ...
        Try
            ...
            'hlamas 30-10-2008
            If bTarifarioGeneral And bContado Then
                txtSub_Total.Text = Format(SubTotal_Costo / (1 + IGV), "###,###,###.00")
                txtMonto_IGV.Text = IGV * SubTotal_Costo
            ...
            If varID = 3 Then
                ...
                If SUB_TOTAL_GENERAL / IIf(Val(PorcentajeDesc) = 0, 1, PorcentajeDesc) < Me.objCargaAsegurada.monto_minimo_PCE And fArticulo = False Then
            ...
            If bMontoMinimo And varID = 3 Then
    ...
    Private Sub btnImprimir_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnImprimir.Click
        ...
        'fnImprimirBOLETA()
        'End If
        imprimir(iDocumento, iFactura)
    End Sub
    Public Sub imprimir(ByVal documento As Integer, ByVal factura As Integer)
        '
        'Dim Cur_DATOS_VENTA As New ADODB.Recordset
        ...
        Dim iSubTotal As Double = 0, iImpuesto As Double = 0, iTotal As Double = 0
        '
        'Por data helper 18/09/2009 '
        '
        'Dim SQLQuery As Object() = {"PKG_IVOVENTACARGA.SP_VER_VENTACONTADO_I", 4, factura.ToString, 2}
        'Dim SQLQuery As Object() = {"SP_VER_VENTACONTADO_I", 4, factura.ToString, 2}
        ...
        If flag Then
            ...
            If documento = 1 Then       'Factura
                ...
                If ldt_cur_datos_venta.Rows(0).Item("es_carga_asegurada") = 1 Then
                    objImprimir.Agregar(290, 108, ldt_cur_datos_venta.Rows(0).Item("agencia_destino") & v_ca)
                Else
                    objImprimir.Agregar(290, 108, ldt_cur_datos_venta.Rows(0).Item("agencia_destino"))
                End If

                'objImprimir.Agregar(290, 75, ObjIMPRESIONFACT_BOL.xNRODOCFAC_BOL)
                objImprimir.Agregar(290, 75, ldt_cur_datos_venta.Rows(0).Item("serie_factura") & "-" & ldt_cur_datos_venta.Rows(0).Item("nro_factura"))

                'objImprimir.Agregar(290, 121, fnGetHora())
                objImprimir.Agregar(290, 121, ldt_cur_datos_venta.Rows(0).Item("hora_factura"))
                '
                objImprimir.Agregar(60, 153, DatePart(DateInterval.Day, ldt_cur_datos_venta.Rows(0).Item("fecha_factura")).ToString.PadLeft(2, "0"))
                objImprimir.Agregar(140, 153, DatePart(DateInterval.Month, ldt_cur_datos_venta.Rows(0).Item("fecha_factura")).ToString.PadLeft(2, "0"))
                objImprimir.Agregar(240, 153, DatePart(DateInterval.Year, ldt_cur_datos_venta.Rows(0).Item("fecha_factura")))

                objImprimir.Agregar(85, 167, ldt_cur_datos_venta.Rows(0).Item("razon_social"))

                'objImprimir.Agregar(85, 184, ObjIMPRESIONFACT_BOL.xDireccionRemitente)
                objImprimir.Agregar(85, 184, ldt_cur_datos_venta.Rows(0).Item("dirorigen"))

                'objImprimir.Agregar(85, 201, ObjIMPRESIONFACT_BOL.xOrigen)
                objImprimir.Agregar(85, 201, ldt_cur_datos_venta.Rows(0).Item("origen"))

                'objImprimir.Agregar(85, 218, ObjIMPRESIONFACT_BOL.xConsignado)
                objImprimir.Agregar(85, 218, ldt_cur_datos_venta.Rows(0).Item("destinatario"))

                'objImprimir.Agregar(85, 235, ObjIMPRESIONFACT_BOL.xDireccionConsignado)
                objImprimir.Agregar(85, 235, ldt_cur_datos_venta.Rows(0).Item("dirdestino"))

                'objImprimir.Agregar(390, 167, ObjIMPRESIONFACT_BOL.xRuc)
                objImprimir.Agregar(390, 167, ldt_cur_datos_venta.Rows(0).Item("Nu_Docu_Suna"))

                'objImprimir.Agregar(390, 201, ObjIMPRESIONFACT_BOL.xDestino)
                objImprimir.Agregar(390, 201, ldt_cur_datos_venta.Rows(0).Item("destino"))

                'objImprimir.Agregar(622, 167, ObjIMPRESIONFACT_BOL.xNroRef)
                objImprimir.Agregar(622, 167, ldt_cur_datos_venta.Rows(0).Item("serie_factura") & "-" & ldt_cur_datos_venta.Rows(0).Item("nro_factura"))

                'objImprimir.Agregar(622, 201, ObjIMPRESIONFACT_BOL.xFormaPago)
                If ldt_cur_datos_venta.Rows(0).Item("Idforma_Pago") = 1 Then
                    objImprimir.Agregar(622, 201, "CONTADO")
                ElseIf ldt_cur_datos_venta.Rows(0).Item("Idforma_Pago") = 2 Then
                    objImprimir.Agregar(622, 201, "CREDITO")
                Else
                    objImprimir.Agregar(622, 201, "PAGO CONTRA ENTREGA")
                End If

                'objImprimir.Agregar(622, 218, ObjIMPRESIONFACT_BOL.xTIPO_ENTREGA)
                If ldt_cur_datos_venta.Rows(0).Item("Idtipo_Entrega") = 1 Then
                    objImprimir.Agregar(622, 218, "AGENCIA")
                Else
                    objImprimir.Agregar(622, 218, "DOMICILIO")
                End If

                'If Not CDbl(ObjIMPRESIONFACT_BOL.xMonto_Sub_Total_Peso) = 0 Then
                objImprimir.Agregar(80, 300, ldt_cur_datos_venta.Rows(0).Item("Cantidad_x_Peso"))
                objImprimir.Agregar(175, 300, "BULTOS")
                objImprimir.Agregar(550, 300, ldt_cur_datos_venta.Rows(0).Item("total_peso"))
                'End If
                'If Not CDbl(ObjIMPRESIONFACT_BOL.xMonto_Sub_Total_Vol) = 0 Then
                objImprimir.Agregar(80, 317, ldt_cur_datos_venta.Rows(0).Item("Cantidad_x_Volumen"))
                objImprimir.Agregar(175, 317, "BULTOS (V.)")
                objImprimir.Agregar(550, 317, ldt_cur_datos_venta.Rows(0).Item("total_volumen"))
                'End If
                'If Not CDbl(ObjIMPRESIONFACT_BOL.xMonto_Sub_Total_Sobre) = 0 Then
                objImprimir.Agregar(80, 334, ldt_cur_datos_venta.Rows(0).Item("Cantidad_x_Sobre"))
                objImprimir.Agregar(175, 334, "SOBRES")
                'End If
                ...
                objImprimir.Agregar(635, 300, ldt_cur_datos_venta.Rows(0).Item("Monto_Sub_Total").ToString.PadLeft(20, " "))

                Dim montoLetras As String = ConvercionNroEnLetras(CDbl(ldt_cur_datos_venta.Rows(0).Item("Total_Costo")))
                objImprimir.Agregar(135, 401, "Son:  " & montoLetras)

                objImprimir.Agregar(270, 487, ldt_cur_datos_venta.Rows(0).Item("usuario"))
                objImprimir.Agregar(460, 455, FormatNumber(dtoUSUARIOS.iIGV, 2))
                ...
                'objImprimir.Agregar(635, 427, ObjIMPRESIONFACT_BOL.xMonto_Sub_Total.PadLeft(20, " "))
                objImprimir.Agregar(635, 427, ldt_cur_datos_venta.Rows(0).Item("Monto_Sub_Total").ToString.PadLeft(20, " "))

                'objImprimir.Agregar(635, 443, Format(CDbl(ObjIMPRESIONFACT_BOL.xMonto_Impuesto), "####,###,###0.00").PadLeft(20, " "))
                objImprimir.Agregar(635, 443, Format(CDbl(ldt_cur_datos_venta.Rows(0).Item("MONTO_IMPUESTO")), "####,###,###0.00").PadLeft(20, " "))

                'objImprimir.Agregar(635, 461, Format(CDbl(ObjIMPRESIONFACT_BOL.xTotal_Costo.ToString), "####,###,###0.00").PadLeft(20, " "))
                objImprimir.Agregar(635, 461, Format(CDbl(ldt_cur_datos_venta.Rows(0).Item("Total_Costo")), "####,###,###0.00").PadLeft(20, " "))

            ElseIf documento = 2 Then   'Boleta
                Dim HoraSistema As String = fnGetHora()

                'objImprimir.Agregar(15, 73, ObjIMPRESIONFACT_BOL.xOrigen)
                objImprimir.Agregar(15, 73, ldt_cur_datos_venta.Rows(0).Item("origen"))
                'objImprimir.Agregar(15, 91, ObjIMPRESIONFACT_BOL.xfecha_factura)
                objImprimir.Agregar(15, 91, ldt_cur_datos_venta.Rows(0).Item("fecha_factura"))
                'objImprimir.Agregar(15, 109, ObjIMPRESIONFACT_BOL.xRazonSocial)
                objImprimir.Agregar(15, 109, ldt_cur_datos_venta.Rows(0).Item("razon_social"))
                'objImprimir.Agregar(15, 127, ObjIMPRESIONFACT_BOL.xConsignado)
                objImprimir.Agregar(15, 127, ldt_cur_datos_venta.Rows(0).Item("destinatario"))
                'objImprimir.Agregar(15, 145, ObjIMPRESIONFACT_BOL.xDireccionConsignado)
                objImprimir.Agregar(15, 145, ldt_cur_datos_venta.Rows(0).Item("dirdestino"))
                'objImprimir.Agregar(162, 73, ObjIMPRESIONFACT_BOL.xDestino)
                objImprimir.Agregar(162, 73, ldt_cur_datos_venta.Rows(0).Item("destino"))
                'objImprimir.Agregar(162, 91, HoraSistema)
                objImprimir.Agregar(162, 91, ldt_cur_datos_venta.Rows(0).Item("hora_factura"))

                'objImprimir.Agregar(290, 91, ObjIMPRESIONFACT_BOL.xFormaPago)
                If ldt_cur_datos_venta.Rows(0).Item("Idforma_Pago") = 1 Then
                    objImprimir.Agregar(290, 91, "CONTADO")
                ElseIf ldt_cur_datos_venta.Rows(0).Item("Idforma_Pago") = 2 Then
                    objImprimir.Agregar(290, 91, "CREDITO")
                Else
                    objImprimir.Agregar(290, 91, "PAGO CONTRA ENTREGA")
                End If
                'objImprimir.Agregar(335, 73, ObjIMPRESIONFACT_BOL.xTIPO_ENTREGA)
                If ldt_cur_datos_venta.Rows(0).Item("Idtipo_Entrega") = 1 Then
                    objImprimir.Agregar(335, 73, "AGENCIA")
                Else
                    objImprimir.Agregar(335, 73, "DOMICILIO")
                End If

                'objImprimir.Agregar(100, 178, "BULTOS   " & Val(Val(ObjIMPRESIONFACT_BOL.Cantidad_x_Peso) + Val(ObjIMPRESIONFACT_BOL.xCantidad_Vol)).ToString)
                objImprimir.Agregar(100, 178, "BULTOS   " & Val(Val(ldt_cur_datos_venta.Rows(0).Item("Cantidad_x_Peso"))) + Val(ldt_cur_datos_venta.Rows(0).Item("Cantidad_x_Volumen")))
                objImprimir.Agregar(100, 193, "SOBRES   " & Val(ldt_cur_datos_venta.Rows(0).Item("Cantidad_x_Sobre")))

                'objImprimir.Agregar(250, 178, ObjIMPRESIONFACT_BOL.xTotalPeso)Total_Peso
                objImprimir.Agregar(250, 178, Val(ldt_cur_datos_venta.Rows(0).Item("total_peso")))

                'objImprimir.Agregar(290, 178, ObjIMPRESIONFACT_BOL.xTotalVolumen)
                objImprimir.Agregar(290, 178, Val(ldt_cur_datos_venta.Rows(0).Item("Total_Volumen")))

                'objImprimir.Agregar(360, 178, Format(CDbl(ObjIMPRESIONFACT_BOL.xTotal_Costo), "###,###,###.00"))
                objImprimir.Agregar(360, 178, Format(CDbl(ldt_cur_datos_venta.Rows(0).Item("Total_Costo")), "###,###,###.00"))

                'objImprimir.Agregar(162, 33, ObjIMPRESIONFACT_BOL.xNRODOCFAC_BOL)
                objImprimir.Agregar(162, 33, ldt_cur_datos_venta.Rows(0).Item("serie_factura") & "-" & ldt_cur_datos_venta.Rows(0).Item("nro_factura"))

                'objImprimir.Agregar(480, 73, ObjIMPRESIONFACT_BOL.xOrigen)
                objImprimir.Agregar(480, 73, ldt_cur_datos_venta.Rows(0).Item("origen"))
                'objImprimir.Agregar(480, 91, ObjIMPRESIONFACT_BOL.xfecha_factura)
                objImprimir.Agregar(480, 91, ldt_cur_datos_venta.Rows(0).Item("fecha_factura"))

                'objImprimir.Agregar(480, 109, ObjIMPRESIONFACT_BOL.xRazonSocial)
                objImprimir.Agregar(480, 109, ldt_cur_datos_venta.Rows(0).Item("razon_social"))

                'objImprimir.Agregar(480, 127, ObjIMPRESIONFACT_BOL.xConsignado)
                objImprimir.Agregar(480, 127, ldt_cur_datos_venta.Rows(0).Item("destinatario"))

                'objImprimir.Agregar(480, 145, ObjIMPRESIONFACT_BOL.xDireccionConsignado)
                objImprimir.Agregar(480, 145, ldt_cur_datos_venta.Rows(0).Item("dirdestino"))

                'objImprimir.Agregar(585, 73, ObjIMPRESIONFACT_BOL.xDestino)
                objImprimir.Agregar(585, 73, ldt_cur_datos_venta.Rows(0).Item("destino"))

                'objImprimir.Agregar(585, 91, HoraSistema)
                objImprimir.Agregar(585, 91, ldt_cur_datos_venta.Rows(0).Item("hora_factura"))

                'objImprimir.Agregar(675, 91, ObjIMPRESIONFACT_BOL.xFormaPago)
                If ldt_cur_datos_venta.Rows(0).Item("Idforma_Pago") = 1 Then
                    objImprimir.Agregar(675, 91, "CONTADO")
                ElseIf ldt_cur_datos_venta.Rows(0).Item("Idforma_Pago") = 2 Then
                    objImprimir.Agregar(675, 91, "CREDITO")
                Else
                    objImprimir.Agregar(675, 91, "PAGO CONTRA ENTREGA")
                End If

                'objImprimir.Agregar(695, 73, ObjIMPRESIONFACT_BOL.xTIPO_ENTREGA)
                If ldt_cur_datos_venta.Rows(0).Item("Idtipo_Entrega") = 1 Then
                    objImprimir.Agregar(695, 73, "AGENCIA")
                Else
                    objImprimir.Agregar(695, 73, "DOMICILIO")
                End If

                'objImprimir.Agregar(360, 238, Format(CDbl(ObjIMPRESIONFACT_BOL.xTotal_Costo), "###,###,###.00"))
                objImprimir.Agregar(360, 238, Format(CDbl(ldt_cur_datos_venta.Rows(0).Item("Total_Costo")), "###,###,###.00"))

                'objImprimir.Agregar(0, 33, ObjIMPRESIONFACT_BOL.xIdFactura)
                objImprimir.Agregar(0, 33, factura)
                ...
                If ldt_cur_datos_venta.Rows(0).Item("es_carga_asegurada") = 1 Then
                    ...
                    'ObtieneMontosAsegurados(iSubTotal, iImpuesto, iTotal)
                    'iTotal = FormatNumber(iTotal, 2)
                    'objImprimir.Agregar(360, 208, Format(CDbl(ldt_cur_datos_venta.Rows(0).Item("Total_Costo")) - CDbl(iTotal), "###,###,###.00"))
                    'objImprimir.Agregar(100, 208, "SEGURO CARGA")
                    'objImprimir.Agregar(250, 208, ldt_cur_datos_venta.Rows(0).Item("usuario"))
                    objImprimir.Agregar(100, 208, ldt_cur_datos_venta.Rows(0).Item("usuario"))
                Else
                    objImprimir.Agregar(100, 208, ldt_cur_datos_venta.Rows(0).Item("usuario"))
                End If
                objImprimir.Agregar(162, 58, ldt_cur_datos_venta.Rows(0).Item("agencia_destino"))

                'objImprimir.Agregar(440, 33, ObjIMPRESIONFACT_BOL.xNRODOCFAC_BOL)
                objImprimir.Agregar(440, 33, ldt_cur_datos_venta.Rows(0).Item("serie_factura") & " " & ldt_cur_datos_venta.Rows(0).Item("nro_factura"))

                'objImprimir.Agregar(440, 48, ObjIMPRESIONFACT_BOL.xIdFactura)
                objImprimir.Agregar(440, 48, factura)

                'objImprimir.Agregar(440, 63, ObjIMPRESIONFACT_BOL.xAgenciaDestino)
                objImprimir.Agregar(440, 63, ldt_cur_datos_venta.Rows(0).Item("agencia_destino"))

                'objImprimir.Agregar(480, 178, "BULTOS   " & Val(Val(ObjIMPRESIONFACT_BOL.xCantidad_Peso) + Val(ObjIMPRESIONFACT_BOL.xCantidad_Vol)).ToString)
                objImprimir.Agregar(480, 178, "BULTOS   " & Val(Val(ldt_cur_datos_venta.Rows(0).Item("Cantidad_x_Peso"))) + Val(ldt_cur_datos_venta.Rows(0).Item("Cantidad_x_Volumen")))

                'objImprimir.Agregar(480, 193, "SOBRES   " & Val(ObjIMPRESIONFACT_BOL.xCantidad_Sobre).ToString)
                objImprimir.Agregar(480, 193, "SOBRES   " & Val(ldt_cur_datos_venta.Rows(0).Item("Cantidad_x_Sobre")))

                'objImprimir.Agregar(580, 178, ObjIMPRESIONFACT_BOL.xTotalVolumen)
                objImprimir.Agregar(580, 178, Val(ldt_cur_datos_venta.Rows(0).Item("Total_Volumen")))

                'objImprimir.Agregar(650, 178, ObjIMPRESIONFACT_BOL.xTotalPeso)
                objImprimir.Agregar(650, 178, Val(ldt_cur_datos_venta.Rows(0).Item("total_peso")))

                'objImprimir.Agregar(700, 178, Format(CDbl(ObjIMPRESIONFACT_BOL.xTotal_Costo), "###,###,###.00"))
                objImprimir.Agregar(700, 178, Format(CDbl(ldt_cur_datos_venta.Rows(0).Item("Total_Costo")), "###,###,###.00"))

                objImprimir.Agregar(480, 208, ldt_cur_datos_venta.Rows(0).Item("usuario"))

                'objImprimir.Agregar(700, 238, Format(CDbl(ObjIMPRESIONFACT_BOL.xTotal_Costo), "###,###,###.00"))
                objImprimir.Agregar(700, 238, Format(CDbl(ldt_cur_datos_venta.Rows(0).Item("Total_Costo")), "###,###,###.00"))
            ...
            ElseIf documento = 3 Then       'guia de envio
                ...
                'objImprimir.Agregar(525, 155, ObjRptGuiaEnvio.P_NROCOMUNICACION_CONTACTO_DESTI)dnidestinatario
                ...
                objImprimir.Agregar(300, 252, "Nº SOBRES " & ldt_cur_datos_venta.Rows(0).Item("Cantidad_x_Sobre").Value)
            End If
            objImprimir.Imprimir()
        Else
            MessageBox.Show("El Documento no tiene asociado una impresora.", "Titán", MessageBoxButtons.OK, MessageBoxIcon.Information)
    ...
    Private Function DevuelveGuiones(ByVal str As String) As Integer
        ...
        Try
            ...
            If flag Then
                ...
                If es_carga_asegurada Then
                    ...
                    'iTotal = FormatNumber(iTotal, 2)
                    'objImprimir.Agregar(360, 208, Format(CDbl(iTotal), "###,###,###.00"))
                    'objImprimir.Agregar(100, 208, "MONTO C.A.")
                Else
                    objImprimir.Agregar(210, 25, ObjRptGuiaEnvio.p_NroGUIA)
                End If
                ...
                'objImprimir.Agregar(20, 210, strNroGuias_Remision)
                objImprimir.Agregar(155, 300, Format(CDate(ObjRptGuiaEnvio.P_FECHA_GUIA), "dd     MM      yy"))
    ...
    Private Sub FrmVentaCargaContado_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        G_Fila = Acceso.ObtieneFila(Me, Me.hnd)
        Acceso.Eliminar(Me, G_Fila)
    End Sub

    Sub ControlaAcceso(ByVal sender As Object, ByVal e As EventArgs) Handles btnNuevo.EnabledChanged, btnAnular.EnabledChanged, btnDevolucion.EnabledChanged, btnVerData.EnabledChanged, btnImprimir.EnabledChanged, btnTIKET.EnabledChanged, btnPCE.EnabledChanged
        If bIngreso Then
    ...
    Sub Grabar()
        Try
            If recuperando_datos_contado And flagPCE = False Then Exit Sub
            If TabMante.SelectedIndex = 0 Then
                SendKeys.Send("{Tab}")
                GoTo FINAL
            End If
