...
Public Class FrmConsulFactuEmiContaFunci
    ...
    Private Sub listar_facturas()
        Try
            ...
            'hlamas 17-09-2015
            If Not IsNothing(Me.cboTipoCartera.SelectedValue) Then
                ObjFactura.TipoCuenta = Me.cboTipoCartera.SelectedValue
            Else
                ObjFactura.TipoCuenta = 0
            End If

            'Mod.19/09/2009 -->Omendoza - Pasando al datahelper   
            ObjFactura.Usuario = dtoUSUARIOS.IdLogin
            ObjFactura.IP = dtoUSUARIOS.IP
            dvListar_facturas = ObjFactura.SP_ConsulFactuEmiContaIII(P_ENTRE)
            '
    ...
    Sub FORMAT_GRILLA3()
        ...
        Dim TOTAL_PESO_volumen As New DataGridViewTextBoxColumn
        With TOTAL_PESO_volumen
            .HeaderText = "Total Peso Volumen"
            .Name = "TOTAL_VOLUMEN"
        ...
        Dim TIPO_CUENTA As New DataGridViewTextBoxColumn
        With TIPO_CUENTA
            .HeaderText = "Tipo Cuenta"
            .Name = "TIPO_CUENTA"
            .DataPropertyName = "TIPO_CUENTA"
            .Width = 100
            .DefaultCellStyle.Alignment = DataGridViewContentAlignment.BottomLeft
            .ReadOnly = True
        End With
        ...
        With DGV3
            .Columns.AddRange(RAZON_SOCIAL, TIPO_COMPROBANTE, SERIE_FACTURA, NRO_FACTURA, FECHA_FACTURA, SIMBOLO_MONEDA, SUB_TOTAL, MONTO_IMPUESTO, TOTAL_COSTO, cantidad_peso, TOTAL_PESO, Cantidad_volumen, TOTAL_PESO_volumen, FORMA_PAGO, LOGIN, ESTADO_REGISTRO, ESTADO_REGISTRO_ENVIO, NOMBRE_AGENCIA, Nombre_Unidad_ORI, Nombre_Unidad_DESTI, TIPO_ENTREGA, FECHA_ENTREGA)
        End With
    End Sub

    Private Sub FrmConsulFactuEmiContaFunci_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Activated
        G_Fila = Acceso.ObtieneFila(Me, Me.hnd)
    End Sub

    Private Sub FrmConsulFactuEmiContaFunci_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        G_Fila = Acceso.ObtieneFila(Me, Me.hnd)
        Acceso.Eliminar(Me, G_Fila)
    End Sub

    Private Sub FrmConsulFactuEmiContaFunci_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Leave

    End Sub

    Private Sub FrmConsulFactuEmiti_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            Me.ShadowLabel1.Text = "Consulta de Ventas Contado y/o Crédito por Funcionario"
            ...
            'hlamas 06-01-2014
            Me.DTPFECHAINICIOFACTURA.Text = Now.ToShortDateString
            Me.DTPFECHAFINALFACTURA.Text = Now.ToShortDateString
            ...
            'Mod.19/09/2009 -->Omendoza - Pasando al datahelper   
            dvlistar_persona_todos = ObjPersona.FNLISTAR_PERSONA_2(ObjPersona.IDTIPO_PERSONA, ObjPersona.IDPERSONA, ObjPersona.CODIGO_CLIENTE)
            ...
            Case 0
                Try
                    ...
                    If Me.cmbProducto.SelectedIndex = 0 Then
                        intProducto = -1
                    Else
                        intProducto = cmbProducto.SelectedValue
                    End If
                    ...
                    With ObjFactura
                        ...
                        ObjReport.printrptB(False, "", "GUI012.RPT", _
    ...
    DTPFECHAFINALFACTURA.GotFocus, _
        ...
        RBAMBOS_INCLU.GotFocus, cboTipoCartera.GotFocus
        ...
        If e.KeyCode = 13 Then
            If Not Iwinlistar_persona_todos.IndexOf(txtidpersona.Text) = -1 Then
                With ObjPersona
                    ...
                    .CODIGO_CLIENTE = "NULL"
                    'Mod.19/09/2009 -->Omendoza - Pasando al datahelper   
                    ObjPersona.FNLISTAR_PERSONA_2(.IDTIPO_PERSONA, .IDPERSONA, .CODIGO_CLIENTE)
    ...
    Private Sub txtCodigoCliente_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtCodigoCliente.KeyPress
        Try
            If e.KeyChar = Chr(13) Then
                ...
                If ObjPersona.FNLISTAR_PERSONA_2(ObjPersona.IDTIPO_PERSONA, ObjPersona.IDPERSONA, ObjPersona.CODIGO_CLIENTE).Count = 1 Then
    ...
    Private Sub DTPFECHAINICIOFACTURA_ValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DTPFECHAINICIOFACTURA.ValueChanged
        Dim intSel As Integer = Me.cmbUsuarios.SelectedValue

        Me.DGV3.DataSource = Nothing

        'hlamas 06-01-2014
        ClsLbTepsa.dtoGENERAL.CarteraResponsable(Me.cmbUsuarios, Me.DTPFECHAINICIOFACTURA.Text, Me.DTPFECHAFINALFACTURA.Text, 2)

        Me.cmbUsuarios.SelectedValue = intSel
    End Sub
    ...
    Private Sub cmbUsuarios_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cmbUsuarios.KeyPress, cboTipoCartera.KeyPress
        If e.KeyChar = Chr(13) Then
    ...
    Private Sub cmbUsuarios_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbUsuarios.SelectedIndexChanged, cboTipoCartera.SelectedIndexChanged
        Me.DGV3.DataSource = Nothing
    End Sub
    ...
    Sub LlenarComboCartera()
        With Me.cboTipoCartera
            ...
            .ValueMember = "id"
            .DataSource = obj.ListarTipoCartera(0)
            .SelectedValue = 0
        End With
    End Sub
End Class
